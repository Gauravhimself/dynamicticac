...
function Tictactoe() {
    ...
    const {board,handleClick,calculateWinner,getStatusMessage,resetGame,setBoard} = useTictac(boardSize)
  ...
  return (
    <div className='game' style={{maxWidth: `calc(${boardSize} * 100px)`}}>
      ...
      <div className='status' style={{maxWidth: `calc(${boardSize} * 100px)`}}>
        <div>
        <div class="relative h-10 w-full min-w-[200px]">
          <input placeholder='Enter Board Size' onChange={(e)=>{setBoardSize(e.target.value)}} 
        ...
        <button className=' mt-3 p-2 px-3' onClick={()=>resetGame()}>Reset Game</button>
        </div>
        <button className='message mt-8q'>{getStatusMessage()}</button>
      </div>
